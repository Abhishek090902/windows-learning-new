...
const AdminLayout = ({ children }: { children: ReactNode }) => {
  ...
  const renderSidebar = (mobile = false) => (
    <aside className={`${mobile ? 'w-[min(86vw,20rem)] h-full' : 'w-72'} admin-sidebar text-white shadow-2xl relative z-50 flex flex-col pt-6 pb-2`}>
      <div className="flex items-center px-5 sm:px-7 mb-8 sm:mb-10 gap-3 group">
        ...
        <div className="min-w-0">
          <h2 className="text-lg sm:text-xl font-bold tracking-tight text-white/95 leading-tight safe-wrap">Windows Learning</h2>
          <p className="text-[10px] text-white/40 uppercase font-semibold tracking-widest leading-none mt-1">Admin Console</p>
        </div>
      </div>

      <div className="px-4 sm:px-6 mb-6 sm:mb-8 group">
        ...
        <div className="admin-glass-card rounded-2xl p-4 shadow-xl border border-white/5">
          <div className="flex items-center gap-3">
            ...
            <Button 
              size="icon" 
              variant="ghost" 
              className="text-white/30 hover:text-red-400 hover:bg-red-400/10 rounded-xl h-10 w-10 transition-all active:scale-95" 
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 dark:bg-[#020817] font-sans antialiased overflow-x-hidden">
      {/* ─── Sidebar ────────────────────────────────────────────────────────────────── */}
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-screen overflow-hidden">
        ...
        <header className="h-16 flex items-center justify-between gap-3 px-4 sm:px-6 lg:px-8 bg-white/60 dark:bg-slate-900/40 backdrop-blur-xl border-b border-slate-200/50 dark:border-slate-800/50 sticky top-0 z-40">
        ...
        <main className="px-4 py-5 sm:px-6 sm:py-6 lg:px-8 lg:py-8 pb-10 h-[calc(100vh-64px)] overflow-y-auto no-scrollbar">

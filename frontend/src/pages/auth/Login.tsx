...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Login failed",
        description: error.response?.data?.error || error.response?.data?.message || "Invalid email or password",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      setIsLoading(true);
      try {
        const response = await api.post('/auth/google', { accessToken: tokenResponse.access_token, role: 'LEARNER' });
        const { user, token } = response.data.data;
        login(user, token);
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Navbar />
      <main className="flex-1 flex items-center justify-center py-16 px-4 login-background">
        <div className="w-full max-w-md bg-card rounded-2xl border shadow-card p-8">
          ...
          <div className="space-y-3 mb-6">
            <button type="button" onClick={() => handleGoogleLogin()} disabled={isLoading} className="w-full flex items-center justify-center gap-3 h-11 rounded-lg border hover:bg-secondary transition-colors text-sm font-medium active:scale-[0.98]">
